...
- 4、点击「发表」按钮时，检测“处理词”（`contentMode=3`），如果存在，则提示修改；没有则提交发表。

## 内容规律

- **艾特**：以 @ 符号开头，空格结尾。
- **话题**：根据配置 [editor.toolbar.hashtag.showMode](../../api/editor/config.md)，值为 1 则是 # 符号开头，空格结尾；值为 2，则是 # 符号开头，# 符号结尾，双 # 号中间可以有单个空格（不能连续空格），但是符号后面或前面不支持有空格。
