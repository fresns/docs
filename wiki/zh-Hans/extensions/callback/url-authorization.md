...
URL 路径凭证参数是完整的 [API Headers](../../api/headers.md) 信息，其中 Headers 的 `X-Fresns-Signature` 参数同 API 的生成规则一致。
...
```json
{
    ...
    "X-Fresns-Signature": "a2bda6bb6f21a3d41c78630401de2c17", // 签名生成规则与 headers 一致
...
经过 Base64 和 URL 编码后的值，便是插件 URL 凭证值。
