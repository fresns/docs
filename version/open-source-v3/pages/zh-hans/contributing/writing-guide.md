# English Docs Writing Guide

> This translation guide is quoted from vuejs.org

[阅读本文的中文版](writing-guide-zh.md)

Writing documentation is an exercise in empathy. We're not describing an objective reality - the source code already does that. Our job is to help shape the relationship between users and the Fresns ecosystem. This ever-evolving guide provides some rules and recommendations on how to do that consistently within the Fresns ecosystem.
...
- **Cookbook:** Recipes in the cookbook are written with some assumption of familiarity with Fresns and its ecosystem. Each is a highly structured document that walks through some common implementation details that a Fresns dev might encounter.
...
- **Unless the context assumes a string template or build system, only write code that works in any environment by the software (e.g. Fresns, Fs, etc).**
- **Show, don't tell.** For example, "To use Fresns on a page, you can add this to your HTML" (then show the script tag), instead of "To use Fresns on a page, you can add a script element with a src attribute, the value of which should be a link to Fresns's compiled source".
...
- **Always prefer simpler, plainer language over complex or jargony language.** For example:
    - "you can use Fresns with a script element" instead of "in order to initiate the usage of Fresns, one possible option is to actually inject it via a script HTML element"
...
![The Serial (Oxford) Comma: When and Why To Use It](https://assets.fresns.com/images/wikis/references/oxford-comma.jpg)
...
We have some dedicated styles to denote something that's worth highlighting in a particular way. These are captured [on this page](https://v2.vuepress.vuejs.org/reference/default-theme/markdown.html#custom-containers). **They are to be used sparingly.**
