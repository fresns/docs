...
[Read the original English version of this article](writing-guide.md)

编写文档是一种换位思考的练习。我们并不是在描述客观现实——源代码已经做到了。我们的工作是帮助塑造用户与 Fresns 生态系统之间的关系。这份不断发展的指南提供了一些规则和建议，说明如何在 Fresns 生态系统中始终如一地做到这一点。
...
- **Cookbook**：Cookbook 中的秘诀是基于对 Fresns 及其生态系统的熟悉程度而编写的。每一个文档都是一个高度结构化的文档，它详细介绍了 Fresns 开发人员可能遇到的一些常见实现细节。
...
- **除非上下文采用字符串模板或构建系统，否则只能编写在软件的任何环境中工作的代码 (例如 Fresns、Fs 等)**
- **显示，不要说**例如，“要在页面上使用 Fresns，可以将其添加到 HTML 中”(然后显示脚本标记)，而不是“要在页面上使用 Fresns，可以添加一个具有 src 属性的脚本元素，该属性的值应为指向 Fresns 编译源的链接”。
...
- **使用牛津逗号** (例如：“a，b，and c” 替换 “a，b and c”)。![为什么牛津逗号很重要](https://assets.fresns.com/images/wikis/references/oxford-comma.jpg)
